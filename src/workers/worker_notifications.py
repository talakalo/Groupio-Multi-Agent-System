...
async def _handle_envelope(data: dict[str, Any]) -> None:
    ...
    if event_name == "notifications.offer_joined_email":
        to_email = payload.get("to_email")
        if not to_email:
            logger.warning("offer_joined_email missing to_email")
            messaging_consumer_messages_total.labels(worker="notifications", result="ignored").inc()
            return
        ...
        messaging_consumer_messages_total.labels(worker="notifications", result="ok").inc()
        return
