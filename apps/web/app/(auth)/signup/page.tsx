...
const signupSchema = z.object({
  ...
  phone: z.string().regex(/^0\d{8,9}$/, 'נא להזין מספר טלפון ישראלי תקין (למשל 0501234567)'),
  password: z.string().min(8, 'סיסמה חייבת להכיל לפחות 8 תווים').regex(/[A-Z]/, 'סיסמה חייבת להכיל לפחות אות גדולה אחת').regex(/[0-9]/, 'סיסמה חייבת להכיל לפחות ספרה אחת'),
...
const ROLE_OPTIONS = [
  { value: 'resident' as UserRole, icon: User, iconBg: 'bg-emerald-50', iconColor: 'text-emerald-600', selectedBg: 'bg-emerald-500', title: 'דייר', description: 'אני דייר בבניין ומחפש להצטרף להצעות קבוצתיות', features: ['גישה להצעות קבוצתיות', 'AI חכם למציאת קבלנים', 'מעקב חסכונות'] },
  { value: 'contractor' as UserRole, icon: Wrench, iconBg: 'bg-sky-50', iconColor: 'text-sky-600', selectedBg: 'bg-sky-500', title: 'קבלן', description: 'אני קבלן ומעוניין להציע שירותים לבניינים', features: ['גישה לביקוש מוכח', 'ניהול הצעות מחיר', 'חשיפה לדיירים חדשים'] },
  { value: 'buildings_manager' as UserRole, icon: ClipboardList, iconBg: 'bg-violet-50', iconColor: 'text-violet-600', selectedBg: 'bg-violet-500', title: 'מנהל בניין', description: 'אני מנהל בניין ורוצה לנהל את הבניין שלי', features: ['ניהול דיירים וועד בית', 'מעקב פניות ותחזוקה', 'גישה לדוחות ומידע'] },
];
...
export default function SignupPage(props: PageParamsProps) {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    setError(null);
    try {
      const response = await apiClient.signup({ name: data.name, email: data.email, phone: data.phone, password: data.password, role: selectedRole === 'buildings_manager' ? 'resident' : selectedRole, buildingId: data.buildingId || undefined });
      useAuthStore.getState().setAccessToken(response.token);
      let user: { role: string } | null = null;
      try {
        const meData = (await apiClient.getMe()) as Record<string, unknown> & { id: string; email: string; role: string; full_name?: string; fullName?: string; phone?: string; preferred_language?: string; preferredLanguage?: string; avatar_url?: string; avatarUrl?: string; building_id?: string; buildingId?: string; contractor_id?: string; contractorId?: string; is_verified?: boolean; isVerified?: boolean; };
        user = { role: meData.role };
        useAuthStore.getState().setUser({ id: meData.id, email: meData.email, fullName: meData.full_name ?? meData.fullName ?? '', phone: meData.phone ?? '', role: coerceAuthStoreRole(meData.role), preferredLanguage: (meData.preferred_language ?? meData.preferredLanguage ?? 'he') as 'he' | 'en', avatarUrl: meData.avatar_url ?? meData.avatarUrl, buildingId: meData.building_id ?? meData.buildingId, contractorId: meData.contractor_id ?? meData.contractorId, isVerified: meData.is_verified ?? meData.isVerified ?? false });
    ...
    <>
      <div className="text-center mb-7">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full mb-5 text-xs font-semibold" style={{ background: 'rgba(26,154,118,0.07)', border: '1px solid rgba(26,154,118,0.14)', color: '#0d6b4f' }}>
          <Shield className="h-3 w-3" /><span>הצטרפו לאלפי שכנים שחוסכים</span>
        </div>
        <h1 className="text-[1.85rem] font-extrabold mb-2" style={{ color: '#0f1f1a', letterSpacing: '-0.03em', lineHeight: '1.15' }}>הצטרפו ל-Groupio</h1>
        <p className="text-[0.9375rem]" style={{ color: '#7a9a8a' }}>התחילו לחסוך עם השכנים שלכם</p>
      </div>

      <StepIndicator steps={[{ label: 'בחירת תפקיד' }, { label: 'פרטים אישיים' }]} currentStep={step === 'role' ? 0 : 1} variant="bar" className="mb-6" />
        ...
        <div className="p-7 pb-8">
          ...
          {step === 'role' && (
            <div className="space-y-3">
              ...
              <button type="button" onClick={() => setStep('details')} className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl transition-colors mt-5" style={{ height: '2.875rem', fontSize: '0.9375rem' }}>
                <span>המשך</span><ArrowLeft className="h-4 w-4 rtl-flip" />
              </button>
            </div>
          )}
          ...
          {step === 'details' && (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div>
                <label htmlFor="name" className="block text-sm font-semibold mb-1.5" style={{ color: '#2d4a40' }}>שם מלא</label>
                <div className="relative">
                  ...
                  <input id="name" type="text" autoComplete="name" placeholder="ישראל ישראלי" className={cn('input-field ps-10', errors.name && 'border-red-400 bg-red-50/30')} aria-describedby={errors.name ? 'name-error' : undefined} aria-invalid={!!errors.name} {...register('name')} />
                </div>
                {errors.name && <p id="name-error" role="alert" className="mt-1.5 text-sm font-medium" style={{ color: '#dc2626' }}>{errors.name.message}</p>}
              </div>

              <div>
                <label htmlFor="email" className="block text-sm font-semibold mb-1.5" style={{ color: '#2d4a40' }}>כתובת אימייל</label>
                <div className="relative">
              ...
              <div>
                <label htmlFor="phone" className="block text-sm font-semibold mb-1.5" style={{ color: '#2d4a40' }}>מספר טלפון</label>
                <div className="relative">
                  ...
                  <input id="phone" type="tel" autoComplete="tel" placeholder="0501234567" className={cn('input-field ps-10', errors.phone && 'border-red-400 bg-red-50/30')} aria-describedby={errors.phone ? 'phone-error' : undefined} aria-invalid={!!errors.phone} {...register('phone')} />
                </div>
                {errors.phone && <p id="phone-error" role="alert" className="mt-1.5 text-sm font-medium" style={{ color: '#dc2626' }}>{errors.phone.message}</p>}
              </div>

              <div>
                <label htmlFor="password" className="block text-sm font-semibold mb-1.5" style={{ color: '#2d4a40' }}>סיסמה</label>
                <div className="relative">
                  ...
                  <input id="password" type="password" autoComplete="new-password" placeholder="לפחות 8 תווים, אות גדולה ומספר" className={cn('input-field ps-10', errors.password && 'border-red-400 bg-red-50/30')} aria-describedby={errors.password ? 'password-error' : undefined} aria-invalid={!!errors.password} {...register('password')} />
                </div>
              ...
              {selectedRole === 'resident' && (
                <div>
                  <label htmlFor="buildingId" className="block text-sm font-semibold mb-1.5" style={{ color: '#2d4a40' }}>קוד בניין (אופציונלי)</label>
                  <div className="relative">
                    ...
                    <input id="buildingId" type="text" autoComplete="off" placeholder="הזינו קוד בניין אם קיבלתם מהוועד" className="input-field ps-10" {...register('buildingId')} />
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-2.5 pt-1">
                ...
                <label htmlFor="tos" className="text-sm" style={{ color: '#6b8c7a' }}>קראתי ומסכים/ה ל<Link href="/terms" className="font-semibold hover:underline mx-1" style={{ color: '#1a9a76' }}>תנאי השימוש</Link>ול<Link href="/privacy" className="font-semibold hover:underline mx-1" style={{ color: '#1a9a76' }}>מדיניות הפרטיות</Link></label>
              </div>

              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setStep('role')} className="flex-1 rounded-xl text-sm font-semibold transition-all" style={{ height: '2.875rem', border: '1.5px solid rgba(26,154,118,0.25)', color: '#0d6b4f', background: 'rgba(26,154,118,0.04)' }}>חזרה</button>
                <button type="submit" disabled={isLoading} className="flex-[2] flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl transition-colors disabled:opacity-70" style={{ height: '2.875rem', fontSize: '0.9375rem' }}>
                  {isLoading ? (<><Loader2 className="h-5 w-5 animate-spin" /><span>נרשם...</span></>) : (<><span>הרשמה</span><ArrowLeft className="h-4 w-4 rtl-flip" /></>)}
                </button>
              </div>
            </form>
          )}
          ...
          <div className="flex items-center gap-3 mt-6">
            ...
            <span className="text-xs font-medium" style={{ color: '#b0c4bc' }}>כבר יש לכם חשבון?</span>
            <div className="flex-1 h-px" style={{ background: 'rgba(10,51,41,0.08)' }} />
          </div>
